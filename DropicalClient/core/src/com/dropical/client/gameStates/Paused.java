...
public class Paused implements GameStatePattern {
    private StateOfGame stateOfGame;
    private TetrisPlayer[] players;

    public Paused(StateOfGame stateOfGame) {
    ...
    public PollRequest buildPollRequest(PollRequest poll) {
        ...
        poll.setGameState(GameState.GAME_PAUSED);

        //anderen Spieler ebenfalls pausieren
        if(players.length == 2) {
            players[(poll.getPlayerNo()+1)%2].getStateOfGame().pauseGame();
        }
