...
public class Lost implements GameStatePattern {
    private StateOfGame stateOfGame;
    private TetrisPlayer[] players;

    public Lost(StateOfGame stateOfGame) {
    ...
    public PollRequest buildPollRequest(PollRequest poll) {
        ...
        poll.setGameState(GameState.GAME_LOST);

        //wenn der Spieler verliert, gewinnt der andere Spieler
        if(players.length == 2) {
            players[(poll.getPlayerNo()+1)%2].getStateOfGame().win();
        }
