...
import com.dropical.client.npc.FishCSS;
...
public class BackgroundCSS {
    //Singleton Stuff
    private static BackgroundCSS ourInstance = new BackgroundCSS(10);
    public static BackgroundCSS getInstance() {
        return ourInstance;
    }
    ...
    //NPCs
    private ArrayList<FishCSS> fishList = new ArrayList<FishCSS>();
    private int fishAmount = 0;

    public BackgroundCSS(int fishAmount) {
...
//            int randomY = (int) Math.floor(Math.random()*300+10);
            int randomY = (int) Math.floor(Math.random()*600+100);
            fishList.add(new FishCSS(world, "NPC/Fish/fish.png", -200, randomY));
        }
    }
